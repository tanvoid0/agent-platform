...
-- Out of alphabetical order on purpose: Postgres resolves a REFERENCES at
-- CREATE TABLE time and SQLite does not, so a table has to exist before the
-- one that points at it. Three pairs are swapped for that reason.
-- **The tables are in dependency order, not alphabetical.** Postgres resolves
-- a REFERENCES at CREATE TABLE time and SQLite does not, so the generated
-- alphabetical order applied fine there and died here with `relation "…"
-- does not exist`. A regenerated file has to be topologically sorted again.
