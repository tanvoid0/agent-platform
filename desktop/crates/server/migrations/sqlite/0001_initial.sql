...
--
-- **Do not edit this file.** It is checksummed: sqlx records the hash when it
-- applies it, and every database that already ran it refuses to start against a
-- changed copy. A schema change is a new `000N_*.sql` beside this one.
--
-- `CREATE TABLE IF NOT EXISTS` throughout, because this runs against databases
-- Alembic already built as well as empty ones.
--
