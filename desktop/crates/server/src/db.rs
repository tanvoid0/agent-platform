...
///
/// **One schema, two dialects.** `migrations/sqlite/` and `migrations/postgres/`
/// hold the same versions with the same descriptions, differing only where the
/// backends genuinely differ (`DATETIME` is not a Postgres type; an
/// `id INTEGER` primary key needs `GENERATED BY DEFAULT AS IDENTITY` there,
/// because Postgres has no rowid to autoincrement from). Both are embedded and
/// the pool's backend picks. A single directory cannot work: the DDL is not
/// portable, which is what blocked Postgres before any query site mattered.
///
...
/// The dialect for a backend. Public so a test can run one against a real
/// database of that kind without going through a pool it has to build first.
pub fn migrator(backend: Backend) -> &'static sqlx::migrate::Migrator {
    match backend {
