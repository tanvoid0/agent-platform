...
/// A database Alembic built, repaired by `0008_timestamp_columns_to_text`.
///
/// **This is the 500 that blocked `POST /api/v1/teams`.** SQLAlchemy's
/// `DateTime` is a `TIMESTAMP` column; `wire::sql_now()` binds a string; and
/// Postgres refuses the pair with "column is of type timestamp without time
/// zone but expression is of type text". Every install created before the Rust
/// server has that shape, and `0001_initial.sql` cannot fix it because its
/// `CREATE TABLE IF NOT EXISTS` skips a table that already exists — which is
/// exactly what this test recreates.
///
/// The assertions are the two halves of "repaired": the column is text now, and
/// the value that was in it survived in the format `sql_now()` writes.
#[tokio::test]
async fn a_legacy_timestamp_column_is_converted_and_then_takes_a_string() {
    let Some(dsn) = dsn() else {
        eprintln!("skipped: no Postgres DSN");
        return;
    };
    let schema = "agent_platform_legacy_test";
    ...
    let _ = admin.execute(format!("DROP SCHEMA {schema} CASCADE").as_str()).await;
    admin.close().await;
    result.expect("the legacy repair");

    async fn drive(url: &str) -> Result<(), String> {
        let pool = db::connect_lazy(url, Backend::Postgres);

        // The Alembic shape: `0001_initial.sql`'s columns, but with the two
        // timestamps as `TIMESTAMP` — and a `DEFAULT CURRENT_TIMESTAMP` on one,
        // because a default is what stops a plain `ALTER … TYPE TEXT` ("default
        // for column cannot be cast automatically").
        pool.execute(
        ...
        .await
        .map_err(|e| format!("create the legacy table: {e}"))?;
        pool.execute(
            "INSERT INTO teamtemplate (name, roster_json, created_at, updated_at) \
             VALUES ('legacy', '{\"roles\":[]}', TIMESTAMP '2026-01-01 09:30:00', \
                     TIMESTAMP '2026-01-01 09:30:00')",
        )
        .await
        .map_err(|e| format!("seed a legacy row: {e}"))?;

        db::ensure_schema(&pool).await.map_err(|e| format!("ensure_schema: {e}"))?;

        let kind: String = sqlx::query_scalar(
            "SELECT data_type FROM information_schema.columns \
             WHERE table_schema = current_schema() AND table_name = 'teamtemplate' \
               AND column_name = 'created_at'",
        )
        .fetch_one(&pool)
        .await
        .map_err(|e| format!("read the column type back: {e}"))?;
        assert_eq!(kind, "text", "the legacy column was not converted");

        // The row that was already there kept its value, in `sql_now()`'s own
        // fixed-width format — a conversion that mangled existing timestamps
        // would pass the type assertion above and still be wrong.
        let kept: String = sqlx::query_scalar("SELECT created_at FROM teamtemplate WHERE name = 'legacy'")
            .fetch_one(&pool)
            .await
            .map_err(|e| format!("read the converted value: {e}"))?;
        assert_eq!(kept, "2026-01-01 09:30:00.000000");

        // And the write that used to 500: a bound Rust string, exactly what
        // `create_team` sends.
        sqlx::query(&db::sql(
            "INSERT INTO teamtemplate (name, roster_json, created_at, updated_at) \
             VALUES (?, ?, ?, ?)",
        ...
        .bind("from the rust server")
        .bind("{\"roles\":[]}")
        .bind(agent_platform_server::wire::sql_now())
        .bind(agent_platform_server::wire::sql_now())
        .execute(&pool)
        .await
        .map_err(|e| format!("the write that used to 500: {e}"))?;

        // Idempotent: the second start runs it against a database already at
        // head, and a converted column must not be found again.
        db::ensure_schema(&pool).await.map_err(|e| format!("second ensure_schema: {e}"))?;
