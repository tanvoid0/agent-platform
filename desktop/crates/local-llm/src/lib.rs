//! In-process llama.cpp chat engine — [ADR 0006](../../../../docs/adr/0006-in-process-rust-core.md)
//! / [ADR 0007](../../../../docs/adr/0007-strangler-rust-server.md).
//!
//! Used by the desktop UI (`agent-platform-desktop`, feature `local-llm`) and by
//! `agent-platformd` `/v1` when a local GGUF is configured. Scope is OpenAI-shaped
//! chat: [`ChatCompletionBody`] in, the same [`ChatChunk`] stream a server SSE
//! relay produces — including tool calls recognised from a JSON object reply.
//!
...
//! full trained context. Here the context is ours to set.
...
/// Engine configuration: GGUF path and context length.
#[derive(Debug, Clone)]
pub struct Config {
...
/// Set by [`override_config`] (tests) — read before [`configure`] and env.
static CONFIG_OVERRIDE: OnceLock<Config> = OnceLock::new();

/// Set once by the host (desktop Settings, daemon flags) via [`configure`].
static CONFIGURED: OnceLock<Config> = OnceLock::new();

/// Point the engine at a model without env or a prior [`configure`]. Test-only:
/// the first call wins, and later ones are ignored rather than swapping a model
/// out from under a loaded context.
#[cfg(test)]
pub fn override_config(model_path: PathBuf, n_ctx: u32) {
    let _ = CONFIG_OVERRIDE.set(Config { model_path, n_ctx });
}

/// Host-supplied config (Settings, CLI). First call wins for the process.
pub fn configure(cfg: Config) {
    let _ = CONFIGURED.set(cfg);
}

/// Resolution: [`CONFIG_OVERRIDE`] → [`configure`] → `LOCAL_MODEL_PATH` /
/// `LOCAL_N_CTX` (default 8192). Env path must be an existing file.
fn config() -> Option<&'static Config> {
    if let Some(cfg) = CONFIG_OVERRIDE.get() {
        return Some(cfg);
    }
    if let Some(cfg) = CONFIGURED.get() {
        return Some(cfg);
    }
    static FROM_ENV: OnceLock<Option<Config>> = OnceLock::new();
    FROM_ENV
        .get_or_init(|| {
            let path = PathBuf::from(std::env::var("LOCAL_MODEL_PATH").ok()?.trim());
            ...
            let n_ctx = std::env::var("LOCAL_N_CTX")
                ...
                .unwrap_or(8192);
            Some(Config { model_path: path, n_ctx })
        })
        .as_ref()
}
...
/// without an async runtime around it.
///
/// # Panics
...
mod tests {
    ...
    ///
    /// ```bash
    /// AGENT_PLATFORM_TEST_GGUF=<path-to.gguf> cargo test -p agent-platform-local-llm --features cuda -- --ignored --nocapture
    /// ```
    #[test]
