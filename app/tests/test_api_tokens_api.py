...
def _create_workspace(c, name="TokenWs", slug=None):
    ...
    if slug:
        body["slug"] = slug
    r = c.post("/api/v1/workspaces/", json=body, headers=_master_headers())
...
def _create_token(c, workspace_id, **kwargs):
    ...
    body.update(kwargs)
    r = c.post(f"/api/v1/workspaces/{workspace_id}/api-tokens/", json=body, headers=_master_headers())
    assert r.status_code == 201, r.text
    return r.json()
...
def test_token_lifecycle_use_then_revoke(client, test_engine):
    ...
    r = c.get(f"/api/v1/projects/{project_id}", headers=token_headers)
    assert r.status_code == 200

    r_revoke = c.post(
        f"/api/v1/workspaces/{ws}/api-tokens/{created['id']}/revoke",
        json={"reason": "rotated"},
        headers=_master_headers(),
    )
    ...
    r2 = c.get(f"/api/v1/projects/{project_id}", headers=token_headers)
...
def test_token_hold_blocks_with_403_then_unhold_restores(client, test_engine):
    ...
    r_hold = c.post(
        f"/api/v1/workspaces/{ws}/api-tokens/{created['id']}/hold",
    ...
    r_unhold = c.post(
        f"/api/v1/workspaces/{ws}/api-tokens/{created['id']}/unhold",
        headers=_master_headers(),
    )
...
def test_token_rate_limit_returns_429(client, test_engine):
    ...
    r1 = c.get(f"/api/v1/projects/{project_id}", headers=token_headers)
    r2 = c.get(f"/api/v1/projects/{project_id}", headers=token_headers)
    r3 = c.get(f"/api/v1/projects/{project_id}", headers=token_headers)
...
def test_token_cannot_manage_other_tokens(client, test_engine):
    ...
    r = c.get(f"/api/v1/workspaces/{ws}/api-tokens/", headers=token_headers)
    assert r.status_code == 403
...
def test_token_scoped_process_isolated_across_workspaces(client, test_engine):
    ...
    tr = c.get("/api/v1/teams/", headers=_master_headers())
    ...
    r_get = c.get(f"/api/v1/processes/{process_id}", headers=token_a_headers)
    ...
    r_cross = c.get(f"/api/v1/processes/{process_id}", headers=token_b_headers)
